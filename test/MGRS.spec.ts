...
describe("MGRS Tests", () => {
  ...
  it("test parse 100k bounds", () => {
    ...
    expect(point.getLongitude()).to.be.approximately(
      comparePoint.getLongitude(),
      0.0001,
    );
    expect(point.getLatitude()).to.be.approximately(
      comparePoint.getLatitude(),
      0.0001,
    );
...
function testCoordinateByPoint(
  ...
  expect(tenKilometerMgrs.coordinate(GridType.TEN_KILOMETER)).to.equal(
    tenKilometer,
  );
  ...
  expect(hundredMeterMgrs.coordinate(GridType.HUNDRED_METER)).to.equal(
    hundredMeter,
  );
